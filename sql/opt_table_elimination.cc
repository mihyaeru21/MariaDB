...
public:
  ...
  /*
    Possible pseudo unique key applicable for this table
...
private:
  void create_pseudo_unique_key_if_needed(TABLE_LIST *table_list,
...
/*
  ...
  DESCRIPTION
    ...
    Create a pseudo unique key if this table is derived and has
...
Dep_analysis_context::create_table_value(TABLE_LIST *table_list)
{
  ...
  create_pseudo_unique_key_if_needed(table_list, tbl_dep);
...
{
  return std::find(derived_table_field_indexes.begin(),
                   derived_table_field_indexes.end(),
                   field_index) !=
      std::end(derived_table_field_indexes);
}
